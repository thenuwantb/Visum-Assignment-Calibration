...
Dual objective funciton will be used. At each iteration, the objective function which provides the highest difference will
be chosen to update the estimates
'''
...
# save results
save_result_path = "E:\\Thenuwan\\Singapore_Calibration\\data\\results\\run_7\\hp_set_13_spsa_13042020_run7.csv"
save_results_all_path = "E:\\Thenuwan\\Singapore_Calibration\\data\\results\\run_7\\hp_set_13_spsa_all_16042020_run7.csv"
...
results_columns = ['in_vehicle', 'transfer_walk', 'origin_wait', 'transfer_wait', 'transfer_penalty', 'pax_trans_total_rmsn',
...
for k in range(max_iterations):
    ...
    for i in range(len(increase_u)):
        if 0.001 < current_estimate[i] + ck * deltaK[i] <= 9.0:
    ...
    for j in range(len(decrease_u)):
        if 0.001 < current_estimate[j] - ck * deltaK[j] <= 9.0:
    ...
    current_objective_function = sgs.select_better_objective_function(inc_obj_list=cost_increase_list,
                                                                      ...
                                                                      obj_func_1=objective_function_1,
                                                                      obj_func_2=objective_function_2)
    ...
    for m in range(len(previous_estimate)):
        if 0.01 <= previous_estimate[m] - gk_step_size[m] <= 9.0:
