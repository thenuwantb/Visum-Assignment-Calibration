...
initial_guess = [1.0, 1.0, 1.0, 300.0]
initial_cost = sgs.run_assignment_calculate_error_stops_pax_trans_combined_2(visum=Visum, estimate_list=initial_guess,
...
for k in range(max_iterations):
    ...
    for i in range(len(increase_u)):
        if 0.0001 < current_estimate[i] + ck * deltaK[i] <= 300.0:
    ...
    for j in range(len(decrease_u)):
        if 0.0001 < current_estimate[j] - ck * deltaK[j] <= 300.0:
    ...
    for m in range(len(previous_estimate)):
        if 0 <= previous_estimate[m] - gk_step_size[m] <= 300.0:
