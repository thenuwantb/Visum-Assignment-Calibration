...
ver_file = "3_HeadwayBased_change_start_time.ver"
version_path = os.path.join(path, ver_file)
Visum = com.Dispatch("Visum.Visum.170")

# save results
save_result_path = "E:\\Thenuwan\\Singapore_Calibration\\data\\results\\hp_set_13_spsa_29022020.csv"

# load visum file
ocv.loadVisum(VisumComDispatch=Visum, verPath=version_path)

# read observed data
observed_stop_df = pd.read_csv("E:\\Thenuwan\\Singapore_Calibration\\data\\observed\\observed_stop_data_all_movements_2902200.csv")
...
initial_cost = sgs.run_assignment_calculate_error_stops_pax_trans_combined(visum=Visum, estimate_list=initial_guess,
...
for k in range(max_iterations):
    ...
    for i in range(len(increase_u)):
        if 0.0001 < current_estimate[i] + ck * deltaK[i] <= 9.9:  # current_estimate[i] + ck * deltaK[i] > 0 and current_estimate[i] + ck * deltaK[i] <= 9.9
            increase_u[i] = current_estimate[i] + ck * deltaK[i]
        else:
            increase_u[i] = current_estimate[i]

    for j in range(len(decrease_u)):
        if 0.0001 < current_estimate[j] - ck * deltaK[j] <= 9.9:
            decrease_u[j] = current_estimate[j] - ck * deltaK[j]
        else:
            decrease_u[j] = current_estimate[j]

    # Step 3 - Function evaluation
    cost_increase = sgs.run_assignment_calculate_error_stops_pax_trans_combined(visum=Visum, estimate_list=increase_u,
                                                                                obs_stops_df=observed_stop_df)
    cost_decrease = sgs.run_assignment_calculate_error_stops_pax_trans_combined(visum=Visum, estimate_list=decrease_u,
                                                                                obs_stops_df=observed_stop_df)
    ...
    for m in range(len(previous_estimate)):
        if 0 <= previous_estimate[m] - gk_step_size[m] <= 9.9:
            current_estimate[m] = previous_estimate[m] - gk_step_size[m]

        else:
            current_estimate[m] = best_estimate[m]

    cost_new = sgs.run_assignment_calculate_error_stops_pax_trans_combined(visum=Visum, estimate_list=current_estimate,
                                                                           obs_stops_df=observed_stop_df)
    # --------------fix 05122019---------------------------------
