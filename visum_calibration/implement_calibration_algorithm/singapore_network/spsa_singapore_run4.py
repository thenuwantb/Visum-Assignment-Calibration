...
The headway calculation method was changed  - with this the unassigned #of trips is reduced
'''
...
# save results
save_result_path = "E:\\Thenuwan\\Singapore_Calibration\\data\\results\\run_3\\hp_set_13_spsa_08042020_run4.csv"
...
initial_cost = initial_cost_dict['mean_pax_trans_combined_rmsn']
...
for k in range(max_iterations):
    ...
    for i in range(len(increase_u)):
        if 0.0001 < current_estimate[i] + ck * deltaK[i] <= 9.0:
    ...
    for j in range(len(decrease_u)):
        if 0.0001 < current_estimate[j] - ck * deltaK[j] <= 9.0:
    ...
    cost_increase_dict = sgs.runAssignmentCalculateErrorRMSN_all_error_terms(Visum=Visum, estimateList=increase_u,
                                                                            obs_stops_df=observed_stop_df,
                                                                            obs_line_routes=observed_line_route_df)
    cost_increase = cost_increase_dict['mean_pax_trans_combined_rmsn']

    cost_decrease_dict = sgs.runAssignmentCalculateErrorRMSN_all_error_terms(Visum=Visum, estimateList=decrease_u,
                                                                             obs_stops_df=observed_stop_df,
                                                                             obs_line_routes=observed_line_route_df)
    cost_decrease = cost_decrease_dict['mean_pax_trans_combined_rmsn']
    ...
    cost_new = cost_new_dict['mean_pax_trans_combined_rmsn']
    # --------------fix 05122019---------------------------------
